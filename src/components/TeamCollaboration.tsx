    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className={`p-6 rounded-2xl border flex flex-col justify-between h-[560px] ${isDarkMode ? 'bg-slate-900/60 border-slate-800/80' : 'bg-white border-slate-200 shadow-sm'}`}>
            
            {/* Header: Cell Annotation Board & Realtime Broadcast Indicator */}
            <div className="border-b border-dashed border-slate-800/50 pb-4 mb-3 flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-mono">Live multi-user message broadcast</p>
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center gap-1 text-[10px] font-mono font-bold px-2.5 py-1 rounded-full border transition-all ${
                  livePulse 
                    ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/50 shadow-sm shadow-emerald-500/20 scale-105' 
            ...
            {/* Cell Quick Tag Mentions Bar */}
            <div className="flex items-center gap-1.5 overflow-x-auto pb-2 mb-2 text-[10px]">
              <span className="text-slate-500 font-mono flex items-center gap-1 shrink-0">
                <Tag className="w-3 h-3 text-blue-400" /> Cell Tag:
              </span>
              {['Row 14', 'Col B', '@admin', 'Slot #Req', '@all'].map((tag) => (
                <button
                  key={tag}
                  type="button"
                  onClick={() => {
                    setSelectedTag(tag);
                    if (!newCommentText.includes(tag)) {
                      setNewCommentText(prev => prev ? `${tag} ${prev}` : `${tag} `);
                    }
                  }}
            ...
            {/* Comments list feed with live broadcast entry animations */}
            <div className="flex-1 overflow-y-auto space-y-3 pr-1 text-xs">
              <AnimatePresence initial={false}>
                {comments.map((comment) => {
                  const isCurrentAuthor = comment.userEmail?.toLowerCase() === currentUserEmail.toLowerCase();
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`p-3 rounded-xl border text-left transition-all ${
                        comment.id === lastReceivedMessageId
                          ? 'bg-blue-500/10 border-blue-500/40 ring-1 ring-blue-500/30'
                          : (isDarkMode ? 'bg-slate-950/40 border-slate-800/80 hover:border-slate-700' : 'bg-slate-50/80 border-slate-200 shadow-2xs')
                      }`}
                    >
                      ...
                      <p className={`mt-1 leading-relaxed ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>
                        {comment.text}
                      </p>

                      <div className="mt-2 pt-1.5 border-t border-slate-800/30 flex items-center justify-between text-[8px] font-mono text-slate-500">
                        <span className="flex items-center gap-1 text-emerald-400/90">
                          <Wifi className="w-2.5 h-2.5 animate-pulse text-emerald-400" /> Broadcasted Live to All Online Users
                        </span>
            ...
            <form onSubmit={handleAddComment} className="mt-3 pt-3 border-t border-dashed border-slate-800/50 flex gap-2">
              <input 
                type="text" 
                required
                value={newCommentText}
                onChange={(e) => setNewCommentText(e.target.value)}
                placeholder={`Post annotation as ${currentUserEmail || 'online team member'}...`}
                className={`flex-1 px-3 py-2 text-xs focus:outline-none border rounded-xl font-medium ${
              ...
              <button 
                type="submit"
                className={`px-3 py-2 text-white font-bold rounded-xl shadow cursor-pointer flex items-center gap-1.5 transition-all hover:scale-102 ${accentClass}`}
                title="Send annotation live to everyone online"
              >
