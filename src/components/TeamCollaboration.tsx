      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className={`p-6 rounded-2xl border ${isDarkMode ? 'bg-slate-900/60 border-slate-800/80' : 'bg-white border-slate-200 shadow-sm'}`}>
            ...
            {/* Column Header Bar */}
            <div className={`hidden md:flex items-center justify-between px-3 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider mb-2 border ${
              isDarkMode 
              ...
              <div className="flex items-center gap-6 shrink-0">
                <span className="w-20 text-center flex items-center justify-center gap-1">
                  <ShieldCheck className="w-3 h-3 text-sky-500" /> Role
                </span>
                <span className="w-24 text-center flex items-center justify-center gap-1">
                  <KeyRound className="w-3 h-3 text-emerald-500" /> Access Status
                </span>
                <span className="w-36 text-center flex items-center justify-center gap-1">
                  <Clock className="w-3 h-3 text-cyan-500" /> Last Active
                </span>
            ...
            <div className={`divide-y text-xs ${isDarkMode ? 'divide-slate-800/60' : 'divide-slate-200'}`}>
              {members
                .filter(m => {
                ...
                .map((m, idx) => {
                  ...
                  return (
                    <div key={m.id} className={`py-3.5 flex flex-col md:flex-row md:items-center justify-between gap-4 px-2 rounded-xl transition-colors ${
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="relative shrink-0">
                          <img 
                            ...
                            alt={m.name} 
                            className={`w-8 h-8 rounded-full object-cover border ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`} 
                            onError={(e) => {
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className={`font-bold truncate ${isDarkMode ? 'text-slate-100' : 'text-slate-900'}`}>{m.name}</span>
                            {isPrimaryOwner && (
                          ...
                          <span className={`text-[10px] block truncate font-mono ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{m.email}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0 flex-wrap">
                        ...
                        <div 
                          className={`group relative flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-[10px] font-mono shrink-0 cursor-help transition-colors ${
                            isDarkMode 
                              ? 'bg-slate-950/80 border-slate-800 hover:border-cyan-500/50' 
                              : 'bg-slate-100 border-slate-200 hover:border-cyan-500/50'
                          }`}
                          title={`Latest Firestore Activity: ${latestAct.action} (${latestAct.timestamp})`}
                        >
                          <Clock className={`w-3 h-3 ${latestAct.hasDoc || m.status === 'active' ? 'text-cyan-400 animate-pulse' : 'text-slate-400'}`} />
                          <span className={`font-sans font-semibold text-[9px] uppercase tracking-wider hidden sm:inline ${
                            isDarkMode ? 'text-slate-400' : 'text-slate-600'
                          }`}>Last Active:</span>
                          <span className={`font-bold ${
                            latestAct.hasDoc || m.status === 'active' 
                              ? 'text-cyan-500' 
                              : isDarkMode ? 'text-slate-400' : 'text-slate-600'
                          }`}>
                            {latestAct.timestamp}
        ...
        {/* Right column: real-time collaboration chatroom */}
