  ...
  return (
    <div className="space-y-8 animate-fadeIn w-full max-w-full overflow-x-hidden">
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          <button 
            ...
            className={`px-4 py-2 text-sm font-semibold border rounded-xl flex items-center gap-2 btn-elevated cursor-pointer ${
              isDarkMode 
          ...
          <button 
            ...
            className={`px-4 py-2 text-sm font-semibold text-white rounded-xl flex items-center gap-2 btn-elevated cursor-pointer ${accentClass}`}
          >
            <Upload className="w-4 h-4" /> Upload CSV
          </button>
          <button 
            onClick={() => onNavigate('insights')}
            className={`px-4 py-2 text-sm font-semibold border rounded-xl flex items-center gap-2 btn-elevated cursor-pointer ${isDarkMode ? 'bg-slate-900 border-slate-800 hover:bg-slate-800 text-slate-200' : 'bg-white border-slate-200 hover:bg-slate-50 text-slate-800'}`}
          >
      ...
      {isOwner && pendingRequests.length > 0 && (
        <div className="p-4 rounded-xl border border-amber-500/40 bg-amber-500/10 text-left space-y-3 card-elevated-1 animate-fadeIn">
              ...
              <div key={req.id} className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 btn-elevated cursor-pointer"
                  >
      ...
      {activePanel === 'records' && (
        <div className={`p-6 rounded-xl border animate-fadeIn transition-all card-elevated-2 ${
          isDarkMode ? 'bg-[#0f172a] border-blue-500/30' : 'bg-blue-50/30 border-blue-200'
      ...
      {activePanel === 'issues' && (
        <div className={`p-6 rounded-xl border animate-fadeIn transition-all card-elevated-2 ${
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              onClick={() => setActivePanel(null)}
              className="text-xs font-bold text-slate-400 hover:text-slate-500 px-2 py-1 rounded hover:bg-slate-500/10 cursor-pointer"
            >
              Close Panel &times;
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 space-y-5">
          {/* Analytics Box 1: 30-Day Audit Trend (Level 2 Elevation) */}
          <div className={`p-5 rounded-xl border flex flex-col justify-between card-elevated-2 ${isDarkMode ? 'bg-[#151D2E] border-[#222F43]' : 'bg-white border-slate-200'}`}>
            <div>
            ...
            <div className={`mt-4 pt-3 border-t flex flex-wrap gap-4 items-center justify-between text-[11px] text-slate-400 ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
              ...
              <button onClick={() => onNavigate('history')} className="text-blue-500 font-bold hover:underline flex items-center gap-1 cursor-pointer">
                View History logs <ArrowRight className="w-3 h-3" />
              </button>
            </div>
          </div>

          {/* Analytics Box 2: 7-Day Rows Processed Recharts Bar Chart (Level 2 Elevation) */}
          <div className={`p-5 rounded-xl border flex flex-col justify-between card-elevated-2 ${isDarkMode ? 'bg-[#151D2E] border-[#222F43]' : 'bg-white border-slate-200'}`}>
            <div>
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-4">
                <div>
        ...
        {/* Files Requiring Attention / Quick Actions list (Level 2 Elevation with Level 3 Upload button) */}
        <div className={`lg:col-span-4 p-5 rounded-xl border flex flex-col justify-between card-elevated-2 ${isDarkMode ? 'bg-[#151D2E] border-[#222F43]' : 'bg-white border-slate-200'}`}>
          <div>
              ...
              {files.length === 0 ? (
                ...
                files.map((file) => (
                  <div 
                    ...
                    className={`p-3 rounded-lg border transition-all cursor-pointer card-hover-lift ${activeFile?.id === file.id ? isDarkMode ? 'bg-blue-500/10 border-blue-500/40' : 'bg-blue-50 border-blue-200' : isDarkMode ? 'bg-[#1a2336] border-[#25334a]' : 'bg-slate-50 border-slate-100'}`}
                  >
          ...
          <div className="mt-4">
            <button 
              onClick={() => onNavigate('upload')}
              className={`w-full py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-1.5 border transition-all cursor-pointer btn-elevated ${isDarkMode ? 'bg-[#0f172a] border-slate-700 text-slate-200 hover:bg-[#1e293b]' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50'}`}
            >
      ...
      {/* Collaboration Timeline Feed (Screen 10 Component - Level 1 Elevation) */}
      <div className={`p-5 rounded-xl border card-elevated-1 ${isDarkMode ? 'bg-[#151D2E] border-[#222F43]' : 'bg-white border-slate-200'}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => onNavigate('team')} className="text-xs text-blue-500 font-bold hover:underline cursor-pointer">
            Manage Team
          </button>
        </div>
