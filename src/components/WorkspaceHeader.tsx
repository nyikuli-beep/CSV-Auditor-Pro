...
export const WorkspaceHeader: React.FC<WorkspaceHeaderProps> = ({
  ...
  const totalUnreadCount = unreadNotificationCount + (isOwner ? pendingSlotRequests.length : 0);
  ...
  return (
    <header className={`h-14 px-3 sm:px-5 border-b flex items-center justify-between gap-2 sm:gap-4 shrink-0 w-full max-w-full relative z-30 transition-colors duration-200 ${
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        ...
        {/* Universal Notifications Bell Button & Popover (Available for Non-Owners and Owners) */}
        <div className="relative" ref={notificationsRef}>
          <button
            ...
            title={totalUnreadCount > 0 ? `${totalUnreadCount} Unread Notifications` : 'Workspace Notifications'}
          >
            <Bell className={`w-4 h-4 ${totalUnreadCount > 0 ? 'text-amber-500' : isDarkMode ? 'text-slate-400' : 'text-slate-600'}`} />
            {totalUnreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-amber-500 text-slate-950 font-black text-[9px] flex items-center justify-center animate-bounce shadow-xs">
                {totalUnreadCount > 9 ? '9+' : totalUnreadCount}
              </span>
            )}
          </button>

          {/* Unified Notification Center Popover */}
          <AnimatePresence>
            {showNotifications && (
              <NotificationCenter
